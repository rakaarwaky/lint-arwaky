...
# ── Violation 1: Function without any type annotations ──
def no_annotations(a, b):
    return a + b


# ── Violation 2: Function with wrong type annotation (declared return int but returns str) ──
def wrong_return_type(x: int, y: int) -> int:
    return f"{x} + {y} = {x + y}"  # ERROR: return str instead of int


# ── Violation 3: Mismatched assignment (declared str then assigns int) ──
...
# ── Violation 6: Accessing nonexistent attribute on object ──
...
# ── Violation 8: Returning Any from a function declared with specific return type ──
def returns_any() -> Any:
    return 99
...
# ── Violation 11: Unsupported operator ──
...
# ── Violation 12: Override with different return type from parent ──
class Parent:
...
# ── Violation 13: Nonexistent TypedDict key ──
...
# ── Violation 15: Invalid type comparison ──
...
# ── Violation 22: Override with different parameter type from parent ──
class BaseProcessor:
...
# ── Violation 23: Nested Any and None propagation ──
...
# ── Violation 24: Invalid operator between str and list ──
...
# ── Violation 25: Variable defined in unused branch ──
