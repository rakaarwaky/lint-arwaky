...
use crate::code_analysis::{compute_score, lint_path, resolve_target, walk_rs_files};
...
pub fn handle_complexity(path: Option<String>) -> ExitCode {
    ...
    for (p, name, comp, _) in functions.iter().take(20) {
        let grade = grade_complexity(*comp);
...
fn grade_complexity(c: usize) -> &'static str {
    if c <= 5 {
        "A"
    } else if c <= 10 {
        "B"
...
pub fn handle_duplicates(path: Option<String>) -> ExitCode {
    let root = resolve_target(path);
...
pub fn handle_trends(path: Option<String>) -> ExitCode {
    let root = resolve_target(path);
