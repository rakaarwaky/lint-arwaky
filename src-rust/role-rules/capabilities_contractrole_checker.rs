...
impl ContractRoleChecker {
    ...
    pub fn check_port(&self) -> Vec<LintResult> { vec![] }
    pub fn check_protocol(&self) -> Vec<LintResult> { vec![] }

    pub fn check_aggregate(&self, file: &str, content: &str, def: &LayerDefinition, violations: &mut Vec<LintResult>) {
        if def.forbidden_inheritance.values.is_empty() { return; }
        ...
        for line in content.lines() {
            ...
            for pattern in &def.forbidden_inheritance.values {
                ...
                let layer_match = lower.contains(&format!("{}::", layer)) || lower.contains(&format!("::{}::", layer));
                if !layer_match { continue; }
                if !suffixes.is_empty() {
                    if !suffixes.iter().any(|s| lower.contains(&format!("_{}", s)) || lower.contains(&format!("::{}", s))) { continue; }
                }
                if let Some(name) = t.split("::").last() {
                    let tn = name.trim_end_matches(';').trim()
                        .trim_start_matches('{').trim_end_matches('}')
                        .split(',').next().unwrap_or("").trim().to_string();
                    if !tn.is_empty() { forbidden_traits.push(tn); }
                }
            }
        }
        for trait_name in &forbidden_traits {
            if content.contains(&format!("impl {} for ", trait_name)) {
                let msg = format!("AES026 FORBIDDEN_INHERITANCE: '{}' implemented from forbidden source.", trait_name);
