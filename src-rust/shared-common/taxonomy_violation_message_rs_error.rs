...
pub enum AesViolation {
    ...
    SuffixMismatch { reason: Option<LintMessage> },
    // AES013 — Forbidden inheritance
    ForbiddenInheritance { reason: Option<LintMessage> },
    // AES014 — Mandatory inheritance
    MandatoryInheritance { reason: Option<LintMessage> },
    // AES015 — Circular import
    CircularImport { reason: Option<LintMessage> },
    // AES020 — File size
    ...
    FixUnusedImport { reason: Option<LintMessage> },
    ...
    HierarchyViolation { reason: Option<LintMessage> },
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::CircularImport { reason } => {
                ...
                let why = reason.as_ref().map(|r| r.to_string()).unwrap_or(default_why);
                write!(f, "AES015 CIRCULAR_IMPORT: Circular dependency detected.\n\
                    WHY? {}\n\
                    FIX: Refactor imports or extract the shared logic into a lower, common layer.", why)
            }
            // AES020
            ...
            Self::CapabilityRouting { struct_name, reason } => {
                ...
                let why = reason.as_ref().map(|r| r.to_string()).unwrap_or(default_why);
                write!(f, "AES0303 CAPABILITY_ROLE: Struct '{}' has no trait implementation.\n\
                    WHY? {}\n\
                    FIX: Implement the capability protocol trait for '{}'.", struct_name, why, struct_name)
            }
            ...
            Self::SurfaceRoleViolation { reason } => {
                ...
                let why = reason.as_ref().map(|r| r.to_string()).unwrap_or(default_why);
                write!(f, "AES0306 SURFACE_ROLE: Surface file exceeds its role mandate.\n\
                    WHY? {}\n\
                    FIX: Divide the surface file into smaller, more specific surface components.", why)
            }
        }
    }
}
