...
pub trait ICheckerAggregate: Send + Sync {
    ...
    // Surface hierarchy
    fn check_surface_hierarchy(&self, files: &[FilePath], root_dir: &FilePath, violations: &mut crate::output_report::taxonomy_result_vo::LintResultList);

    // Orphan detection
    fn build_orphan_graph_context(
        &self, files: &[String], root_dir: &str,
