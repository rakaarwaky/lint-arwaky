...
impl IDispatchRoutingParserProtocol for DispatchRoutingParser {
    fn strip_docstrings(&self, text: &str) -> String {
        // Simple stub: remove triple-quoted Python docstrings
        let re = regex::Regex::new(r#""""[\s\S]*?""""|'''[\s\S]*?'''"|#[^\n]*"#).unwrap();
        re.replace_all(text, "").to_string()
    }

    fn extract_class_methods(&self, text: &str) -> crate::taxonomy::ClassDefinitionMap {
        use std::collections::HashMap;
        let mut defs: HashMap<String, crate::taxonomy::ClassMethodsVO> = HashMap::new();
        ...
        for line in text.lines() {
            if let Some(c) = class_re.captures(line) {
                ...
                defs.entry(c[1].to_string()).or_insert_with(|| crate::taxonomy::ClassMethodsVO { methods: vec![] });
            } else if let Some(m) = method_re.captures(line) {
                if let Some(ref cls) = current_class {
                    if let Some(entry) = defs.get_mut(cls) {
                        entry.methods.push(m[1].to_string());
                    }
                }
            }
        }
