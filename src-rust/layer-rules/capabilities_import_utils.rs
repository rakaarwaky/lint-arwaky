...
pub fn import_matches_scope(import_line: &str, layer: &str, suffixes: &[&str]) -> bool {
    ...
    suffixes.iter().any(|s| {
        segments.iter().any(|seg| {
            let cleaned = seg.trim_end_matches(';').trim().trim_start_matches('{').trim_end_matches('}').trim();
            cleaned.split(',').any(|t| {
                let name = t.trim();
                let name_lower = name.to_lowercase();
                if name_lower.ends_with(&format!("_{}", s)) { return true; }
                if let Some(rest) = name_lower.strip_suffix(s) {
                    if rest.is_empty() || rest.ends_with('_') { return true; }
                    if name.len() >= s.len() {
                        let suffix_in_orig = &name[name.len() - s.len()..];
                        if suffix_in_orig.starts_with(|c: char| c.is_uppercase()) { return true; }
                    }
                }
                false
            })
...
pub fn get_basename(file: &str) -> String {
    Path::new(file).file_name().and_then(|f| f.to_str()).unwrap_or("").to_string()
}

pub fn read_import_lines(file: &str) -> Vec<(usize, String)> {
    let Ok(content) = fs::read_to_string(file) else { return vec![]; };
...
pub fn parse_import_lines(content: &str) -> Vec<(usize, String)> {
    ...
    while i < lines.len() {
        ...
        if trimmed.starts_with("import ") || trimmed.starts_with("from ") || trimmed.starts_with("extern crate ") {
        ...
        if trimmed.starts_with("use ") {
            ...
            if combined.contains('{') && !combined.contains('}') {
                ...
                while i < lines.len() {
                    ...
                    if part.contains('}') || combined.ends_with(';') { break; }
                    i += 1;
                }
                ...
                while i + 1 < lines.len() {
                    ...
                    if next.starts_with("use ") || next.is_empty() { break; }
                    combined.push_str(&format!(" {}", next));
                    if next.ends_with(';') { i += 1; break; }
                    i += 1;
                }
                combined = combined.split_whitespace().collect::<Vec<&str>>().join(" ");
...
pub fn extract_layer_from_import(segment: &str) -> Option<String> {
    const PREFIX_MAP: &[(&str, &str)] = &[
        ("taxonomy_", "taxonomy"), ("contract_", "contract"),
        ("capabilities_", "capabilities"), ("infrastructure_", "infrastructure"),
        ("agent_", "agent"), ("surface_", "surfaces"),
    ];
    for (prefix, layer) in PREFIX_MAP {
        if segment.starts_with(prefix) { return Some(layer.to_string()); }
    }
    match segment {
        "taxonomy" => Some("taxonomy"), "contract" => Some("contract"),
        "capabilities" => Some("capabilities"), "infrastructure" => Some("infrastructure"),
        "agent" => Some("agent"), "surfaces" | "surface" => Some("surfaces"),
        _ => None,
    }.map(|s| s.to_string())
}
