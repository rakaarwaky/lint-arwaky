...
mod tests {
    ...
    #[test]
    fn test_parse_config_mandatory() {
        let config = default_aes_config();
        eprintln!("Total rules: {}", config.rules.len());
        let mut found_mandatory = false;
        for r in &config.rules {
            if !r.mandatory.values.is_empty() {
                eprintln!("  Rule[{}] scope={} mandatory={:?}", r.name.value, r.scope.value, r.mandatory.values);
                found_mandatory = true;
            }
        }
        assert!(found_mandatory, "No rules with mandatory imports found!");
    }
}
...
mod tests2 {
    ...
    fn test_parse_config_debug() {
        ...
        let arch = raw.get("architecture").unwrap();
        let json = serde_json::to_value(arch).unwrap();
        
        // Check rules in raw JSON
        if let Some(rules) = json.get("rules") {
            eprintln!("rules type: {:?}", rules);
            if let Some(obj) = rules.as_object() {
                eprintln!("rules keys: {:?}", obj.keys().collect::<Vec<_>>());
                for (k, v) in obj {
                    eprintln!("  rule[{}]: {:?}", k, v.get("scope"));
                }
            }
        } else {
            eprintln!("No 'rules' key in architecture!");
        }
    }
}
