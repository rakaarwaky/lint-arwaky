...
class Notify:
    ...
    def log_event(self, event: str, message: str) -> None:
        self.logger.write(event.split(":")[0], event, message)
    ...
    def pipeline_started(self, feature: str, pipeline_id: str) -> None:
        self.log_event("pipeline_started", f"Pipeline {pipeline_id} started for feature: {feature}")

    def pipeline_completed(self, feature: str, pipeline_id: str, pr_number: str = "") -> None:
        self.log_event("pipeline_completed", f"Pipeline {pipeline_id} completed for feature: {feature}")
        if pr_number:
            self.pr_comment(pr_number, f"✅ Pipeline completed for feature: {feature}")

    def qa_approved(self, feature: str, pr_number: str) -> None:
        self.log_event("qa_approved", f"Quality-Analysis approved PR #{pr_number} for feature: {feature}")
