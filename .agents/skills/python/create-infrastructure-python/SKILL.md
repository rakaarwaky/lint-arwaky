...
**This skill consolidates rules from:** `fix-capability-structure`, `create-missing-protocols` (expanded to ports), `module_logic_validator`, `enforce-1-class-per-file`, and `method_classifier` — applied specifically to the infrastructure layer.
