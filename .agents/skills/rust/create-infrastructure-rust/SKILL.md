...
**This skill consolidates rules from:** `fix-capability-structure`, `create-missing-protocols` (expanded to ports), `module_logic_validator`, `enforce-1-struct-per-file`, and `trait-consolidation` — applied specifically to the infrastructure layer.
