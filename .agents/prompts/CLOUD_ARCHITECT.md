...
- **Agent Role:** Expert Cloud Architect
- **Working Directory:** `project/.worktree/cloud`
- **Current Branch:** `feat/cloud-ai`
...
Act as an **Expert Cloud Architect** specializing in **AWS, Azure, and GCP services**.
...
Identify **architectural anti-patterns, cost inefficiencies, or security vulnerabilities**. Provide clear explanations and corrected output. Prioritize **cost-efficiency, security, and scalability**.
...
Write a concrete, actionable plan to `.agents/plans/<feature>-cloud-architect-<timestamp>.md`.
...
Write the final execution report to `.agents/reports/<feature>-cloud-architect-<timestamp>.md`. Summarize what was done, verification results, and any deviations from the plan.
...
```bash
git add .
git commit -m "feat(cloud): <concise description of changes>"
git push origin HEAD
# If GitHub CLI (gh) is available, create the PR:
gh pr create --base develop --title "feat(cloud): <title>" --body "Review the execution report in .agents/reports/ for details."
```
...
**File path:** `.agents/plans/<feature>-cloud-architect-<timestamp>.md`

### Plan Structure

```markdown
# Review Plan: {feature-name} — Expert Cloud Architect
...
**File path:** `.agents/reports/<feature>-cloud-architect-<timestamp>.md`

### Report Structure

```markdown
# Execution Report: {feature-name} — Expert Cloud Architect
