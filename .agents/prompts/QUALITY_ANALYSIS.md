...
- **Agent Role:** Expert Software QA and Developer
- **Working Directory:** `project/.worktree/qa`
- **Current Branch:** `feat/qa-ai`
...
Act as an **Expert Software QA and Developer** specializing in **functional and performance quality**.
...
Identify **functional bugs and performance bottlenecks**. Provide clear explanations and corrected output. Prioritize **correctness and performance**.
...
Write a concrete, actionable plan to `.agents/plans/<feature>-quality-analysis-<timestamp>.md`.
...
Write the final execution report to `.agents/reports/<feature>-quality-analysis-<timestamp>.md`. Summarize what was done, verification results, and any deviations from the plan.
...
```bash
git add .
git commit -m "feat(qa): <concise description of changes>"
git push origin HEAD
# If GitHub CLI (gh) is available, create the PR:
gh pr create --base develop --title "feat(qa): <title>" --body "Review the execution report in .agents/reports/ for details."
```
...
**File path:** `.agents/plans/<feature>-quality-analysis-<timestamp>.md`

### Plan Structure

```markdown
# Review Plan: {feature-name} — Expert Software QA and Developer
...
**File path:** `.agents/reports/<feature>-quality-analysis-<timestamp>.md`

### Report Structure

```markdown
# Execution Report: {feature-name} — Expert Software QA and Developer
