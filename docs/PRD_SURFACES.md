...
**Dependency Rule**: Can import from `agent`, `taxonomy`, `contract(aggregate)`, and sibling surfaces only. Three surface categories with different rules:

- **Smart Surfaces** (`_page`, `_command`, `_handler`, `_controller`, `_entry`): import agent + taxonomy + contract(aggregate)
- **Utility Surfaces** (`_hook`, `_store`, `_provider`, `_router`): import taxonomy + contract(aggregate) + passive surfaces
...
| **Utility Surfaces** (helpers)    | `_hook`, `_store`, `_provider`, `_router`                  | Import: taxonomy, contract(aggregate), passive surfaces. Forbidden: infrastructure, capabilities, contract(port), contract(protocol), smart surfaces |
...
YAML config defines these surface suffixes: `_command`, `_handler`, `_controller`, `_page`, `_view`, `_component`, `_router`, `_layout`, `_entry`, `_hook`, `_store`, `_provider`
...
### Smart Surfaces (page|command|handler|controller|entry)

| Rule              | Setting                                                                                               |
| ----------------- | ----------------------------------------------------------------------------------------------------- |
| Allowed imports   | `taxonomy`, `contract(aggregate)`, `surfaces` (siblings)                                        |
| Mandatory imports | `taxonomy`, `contract(aggregate)` (AES002)                                                        |
| Forbidden imports | `agent`, `infrastructure`, `capabilities`, `contract(port)`, `contract(protocol)`, `root` |
| AES001            | Smart surfaces must NOT import concrete agent, infra, or capability modules directly                  |
| AES023            | Surfaces access capabilities/infrastructure only through ServiceContainerAggregate                    |

### Utility Surfaces (hook|store|provider|router)

| Rule              | Setting                                                                                                                    |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------- |
| Allowed imports   | `taxonomy`, `contract(aggregate)`, `surfaces(passive)`                                                               |
| Forbidden imports | `agent`, `infrastructure`, `capabilities`, `contract(port)`, `contract(protocol)`, `surfaces(smart)`, `root` |
| AES018            | Utility surfaces cannot depend on Smart surfaces — must be stateless helpers                                              |

### Passive Surfaces (component|layout|view)

| Rule              | Setting                                                                                       |
| ----------------- | --------------------------------------------------------------------------------------------- |
| Allowed imports   | `taxonomy`                                                                                  |
| Mandatory imports | `taxonomy` (AES002)                                                                         |
| Forbidden imports | `agent`, `contract`, `infrastructure`, `capabilities`, `surfaces(active)`, `root` |
| AES019            | Passive surfaces must only import Taxonomy — no logic or orchestration                       |

### Primitive Policy

| Scope        | `no_primitives`                                    |
| ------------ | ---------------------------------------------------- |
| `surfaces` | `false` — may use primitive types for I/O parsing |

### Barrel (AES012)

`mod.rs` must export all public commands and handlers. ZERO `pub mod`/`pub use` in non-barrel sub-modules (AES013).

## 5. Architectural Rules

| Rule   | Constraint                                                                                                                                                             |
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AES001 | Zero imports to capabilities or infrastructure                                                                                                                         |
| AES002 | Smart surfaces must import `taxonomy` + `contract(aggregate)`; passive surfaces must import `taxonomy`                                                           |
| AES011 | Suffixes:`_command`, `_handler`, `_controller`, `_page`, `_view`, `_component`, `_router`, `_layout`, `_entry`, `_hook`, `_store`, `_provider` |
...
| AES019 | Passive surfaces must only import Taxonomy — zero domain logic                                                                                                        |
| AES023 | Zero imports to capabilities or infrastructure — use ServiceContainerAggregate                                                                                        |
...
- Utility surfaces do NOT import Smart surfaces (AES018)
- Passive surfaces import taxonomy ONLY (AES019)
