...
> The current reference implementation of AES is **lint-arwaky** (a Rust project). The architecture itself is language-agnostic; the suffix conventions and examples below are written with the Rust implementation in mind but apply equally to Python, TypeScript, or Go.
...
%%{init: {'theme': 'default', 'themeVariables': { 'primaryColor': '#ffffff', 'primaryTextColor': '#000000', 'primaryBorderColor': '#333333', 'lineColor': '#333333' }}}%%
graph TD
    subgraph "Violations (❌ Forbidden Imports)"
        direction LR
        V1["Surfaces"] -.->|"❌"| VC["Capabilities"]
        V2["Surfaces"] -.->|"❌"| VI["Infrastructure"]
        V3["Surfaces"] -.->|"❌"| VA["Agent"]
        V4["Capabilities"] -.->|"❌"| VI2["Infrastructure"]
        V5["Infrastructure"] -.->|"❌"| VC2["Capabilities"]
        V6["Contract"] -.->|"❌"| VC3["Capabilities"]
        V7["Contract"] -.->|"❌"| VI3["Infrastructure"]
        V8["Taxonomy"] -.->|"❌"| VAny["Any Outer Layer"]
    end

    subgraph "Allowed (✅ Valid Imports)"
        direction LR
        A1["Surfaces"] -->|"✅"| ACT["Contract"]
        A2["Surfaces"] -->|"✅"| AT["Taxonomy"]
        A3["Agent"] -->|"✅"| AI["Infrastructure"]
        A4["Agent"] -->|"✅"| AC["Capabilities"]
        A5["Capabilities"] -->|"✅"| ACT2["Contract"]
        A6["Infrastructure"] -->|"✅"| ACT3["Contract"]
        A7["Contract"] -->|"✅"| AT2["Taxonomy"]
    end
```
...
- **Components**:
  ...
  - **Aggregate (`_aggregate`)**: Composition-based facades grouping related ports/protocols. Forbidden to inherit from Port/Protocol directly (**AES026**). _Ex: `service_container_aggregate.rs`_
...
- **Components**:
  - **Container (`_container`)**: Purely structural DI wiring. Zero business logic (**AES021**). _Ex: `dependency_injection_container.rs`_
  ...
  - **Registry (`_registry`)**: Thread-safe, passive inventory store for CRUD/state. _Ex: `agent_job_registry.rs`_
