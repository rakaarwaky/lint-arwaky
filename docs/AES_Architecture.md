...
```mermaid
graph TD
    S["Surfaces - CLI, MCP Server, API"]
    A["Agent - DI Container, Orchestrators"]
    C["Capabilities - Checkers, Analyzers"]
    I["Infrastructure - Adapters, Scanners"]
    CT["Contract - Ports, Protocols"]
    T["Taxonomy - VOs, Entities, Errors"]

    S -->|"delegates via contract traits"| A
    A -->|"imports"| C
    A -->|"imports"| I
    C -->|"imports"| CT
    I -->|"imports"| CT
...
> **Note**: Surfaces access Agent capabilities exclusively through **contract traits** (e.g., `ServiceContainerAggregate`, `IArchLintProtocol`). Direct `use agent::*` imports are forbidden (**AES023**). The runtime delegation flow goes Surface → Contract trait → Agent implementation, but at the import/module level, Surfaces may only reference `taxonomy` and `contract`.

#### Violation Detection

```mermaid
graph TD
    V1["Surfaces to Capabilities"]
    V2["Surfaces to Infrastructure"]
    V3["Surfaces to Agent"]
    V4["Capabilities to Infrastructure"]
    V5["Infrastructure to Capabilities"]
    V6["Contract to Capabilities"]
    V7["Contract to Infrastructure"]
    V8["Taxonomy to Any outer layer"]

    A1["Surfaces to Contract"]
    A2["Surfaces to Taxonomy"]
    A3["Agent to Infrastructure"]
    A4["Agent to Capabilities"]
    A5["Capabilities to Contract"]
    A6["Infrastructure to Contract"]
    A7["Contract to Taxonomy"]
```
...
sequenceDiagram
    ...
    Note over U,MC: User runs: auto-linter check .
...
To solve the "Scattered Feature Problem" inherent in layered architectures, AES abandons deep physical folder nesting in favor of a **3-Word File Naming Convention**. Every file acts as a 3D coordinate system: `[prefix]_[middle]_[suffix].py`.
...
_Example:_ `auth_session_port.py` instantly tells us the domain (auth), the concept (session), and the architectural role (port).

---

## Detailed Layer Specifications

Listed from the innermost (core) to the outermost (edge) layer. The **Crate Root Kernel** is the implicit meta-layer that hosts cross-cutting utilities (macros, kernels, foundational bootstrapping) consumed by every architectural layer.

### 0. Crate Root Kernel: Cross-Cutting Utilities

- **Path**: `src-rust/` (crate root, _not_ inside any of the 6 layers)
- **Allowed Suffixes**: `_entry`, `_kernel`
- **Allowed Imports**: Anything (`crate::*`), since root utilities are pre-runtime compile-time helpers. Imports must never reach _down_ from a layer _into_ a root utility; root utilities are pulled _up_ via `use` or `#[macro_export]`.
- **Description**: Houses foundational, cross-cutting utilities that do not fit any single layer. This is the only category permitted to live outside the 6-layer structure. Macros, kernels, and bootstrap helpers belong here.
- **Components**:
  - **Entry Point (`_entry`)**: Binary or library composition root. _Ex: `cli_main_entry.rs`, `mcp_main_entry.rs`_
  - **Kernel (`_kernel`)**: Foundational utilities, code-generation macros, and kernels whose behavior is consumed by multiple layers via `#[macro_export]` or re-exports. Must follow the 3-word naming convention (`prefix_middle_kernel.rs`). The kernel file itself is exempt from the AES004/AES005 line-count rules when its content is purely declarative (macros, type aliases). _Ex: `common_vo_kernel.rs` (provides `impl_string_vo!` macro for Value Object boilerplate)_

#### Crate Root Import Rules

- Root utilities **never** live inside a layer folder. Putting `common_vo_kernel.rs` under `taxonomy/` would falsely imply it is a taxonomy-layer concern; it is a cross-cutting helper, not domain code.
- A kernel file's macros are made available to the rest of the crate via `#[macro_export]`, which automatically places them at the crate root namespace. Callers invoke them as `crate::macro_name!(...)` regardless of the kernel file's physical location.
- Adding a new kernel file requires updating `auto_linter.config.rust.yaml` `architecture.layers.root.suffix` to include the `_kernel` suffix (already done).
...
- **Components**:
  - **Port (`_port`)**: Outbound interfaces for technical operations (I/O, DB, Network). Implemented by Infrastructure. _Ex: `file_system_port.py`_
  - **Protocol (`_protocol`)**: Inbound interfaces for use cases or domain calculations. Implemented by Capabilities. _Ex: `arch_rule_protocol.py`_
  - **Aggregate (`_aggregate`)**: Composition-based facades grouping related ports/protocols. Forbidden to inherit from Port/Protocol directly (**AES026**). _Ex: `service_container_aggregate.py`_
...
- **Components**:
  - **Checker/Analyzer (`_checker`, `_analyzer`)**: Evaluates specific audit rules. _Ex: `arch_import_checker.py`_
  - **Processor/Resolver (`_processor`, `_resolver`)**: Orchestrates transformations or graph operations. _Ex: `orphan_graph_resolver.py`_
  - **Evaluator (`_evaluator`)**: Coordinates multiple checkers to score complex rules. _Ex: `architecture_rule_evaluator.py`_
...
- **Components**:
  - **Adapter (`_adapter`)**: Implements concrete ports for external tools. _Ex: `python_ruff_adapter.py`_
  - **Scanner (`_scanner`)**: Interfaces with raw hardware/platform APIs. _Ex: `os_fs_scanner.py`_
  - **Provider (`_provider`)**: Delivers technical configuration or utilities. _Ex: `config_yaml_provider.py`_
...
- **Components**:
  - **Container (`_container`)**: Purely structural DI wiring. Zero business logic (**AES021**). _Ex: `dependency_injection_container.py`_
  - **Orchestrator (`_orchestrator`)**: Conducts sequential flow for a single domain goal. Must be completely stateless between calls (**AES021**). _Ex: `arch_compliance_orchestrator.py`_
  - **Coordinator (`_coordinator`)**: Orchestrates high-level policies across multiple orchestrators. _Ex: `arch_compliance_coordinator.py`_
  - **Registry (`_registry`)**: Thread-safe, passive inventory store for CRUD/state. _Ex: `agent_job_registry.py`_
  - **Manager (`_manager`)**: Supervises lifecycles and background runners. _Ex: `lifecycle_state_manager.py`_
...
graph TB
    subgraph "Binary Entry Point (Composition Root)"
        BE["cli_main_entry.rs<br/>mcp_main_entry.rs"]
    end
...
**Key Principle**: Surface interacts with **two traits only**:
1. `ServiceContainerAggregate` → to get capabilities
2. `IArchLintProtocol` → to execute lint

Surface **never** imports `DependencyInjectionContainer` (Agent) or `ArchLintHandler` (Capabilities).
...
- **Components**:
  - **Smart Surfaces**: Parse input, delegate to Agent orchestrators, return structured output. _Ex: `cli_check_command.py`, `mcp_server_handler.py`_
  - **Passive Surfaces**: Dumb, presentation-only components. Receive read-only VOs, never import agents/contracts (**AES019**). _Ex: `dashboard_view.py`_
