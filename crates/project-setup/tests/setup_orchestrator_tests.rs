...
use shared::project_setup::taxonomy_setup_contract_vo::{TransportProtocol, TransportUrlVO};
...
impl shared::project_setup::contract_setup_protocol::ISetupManagementProtocol for MockProtocol {
    fn generate_env(&self, _home: &DirectoryPath) -> shared::project_setup::taxonomy_setup_contract_vo::EnvContentVO {
        shared::project_setup::taxonomy_setup_contract_vo::EnvContentVO::new("MOCK=true\n")
    }
    fn generate_mcp_config(&self) -> shared::project_setup::taxonomy_setup_contract_vo::McpConfigVO {
        let mut config = std::collections::HashMap::new();
        config.insert("lint-arwaky".to_string(), serde_json::json!({"command": "mock"}));
        shared::project_setup::taxonomy_setup_contract_vo::McpConfigVO::new(config)
    }
    fn mcp_config_claude(&self) -> shared::project_setup::taxonomy_setup_contract_vo::McpConfigVO { self.generate_mcp_config() }
    fn mcp_config_hermes(&self) -> shared::project_setup::taxonomy_setup_contract_vo::McpConfigVO { self.generate_mcp_config() }
    fn mcp_config_vscode(&self) -> shared::project_setup::taxonomy_setup_contract_vo::McpConfigVO { self.generate_mcp_config() }
    ...
    async fn install_python_adapters(&self) -> shared::mcp_server::taxonomy_job_vo::SuccessStatus { shared::mcp_server::taxonomy_job_vo::SuccessStatus::new(true) }
    async fn install_javascript_adapters(&self, _sudo: bool) -> shared::mcp_server::taxonomy_job_vo::SuccessStatus { shared::mcp_server::taxonomy_job_vo::SuccessStatus::new(true) }
