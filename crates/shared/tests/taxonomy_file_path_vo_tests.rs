...
fn creates_from_root_path() {
    let fp = FilePath::new("/home/user/project/src/lib.rs").unwrap();
...
fn file_path_with_special_chars() {
    let fp = FilePath::new("my project/src/utils@2.0/file.test.rs").unwrap();
    assert_eq!(fp.value, "my project/src/utils@2.0/file.test.rs");
}

#[test]
fn file_path_newtype_pattern() {
    let fp = FilePath::new("config.yaml").unwrap();
