...
pub fn default_aes_config() -> ArchitectureConfig {
    DEFAULT_RUST_CONFIG
        .get_or_init(|| parse_config_yaml(include_str!(concat!(env!("OUT_DIR"), "/lint_arwaky.config.rust.yaml"))))
        .clone()
}
...
pub fn default_config_for_language(language: &str) -> ArchitectureConfig {
    match language {
        ...
        "python" => DEFAULT_PYTHON_CONFIG
            .get_or_init(|| {
                parse_config_yaml(include_str!(concat!(env!("OUT_DIR"), "/lint_arwaky.config.python.yaml")))
            })
            .clone(),
