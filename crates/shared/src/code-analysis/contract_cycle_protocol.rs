...
impl ICycleAnalysisProtocol for DefaultCycleAnalysisProtocol {
    async fn check_cycles(
        ...
        for file in &files.values {
            ...
            if let Some(cycle) =
                find_cycle_dfs(&file_str, &adjacency, &mut global_visited, &mut in_stack, &mut path)
            {
...
mod tests {
    ...
    use crate::code_analysis::taxonomy_import_source_vo::{ImportInfo, ImportInfoList, PrimitiveViolationList};
    use crate::pipeline_jobs::taxonomy_job_vo::{ResponseData, SuccessStatus};
    ...
    impl ISourceParserPort for MockSourceParserPort {
        ...
        fn get_raw_symbols(&self, _path: &FilePath) -> Result<ResponseData, SourceParserError> { Ok(ResponseData { value: None, stdout: String::new(), stderr: String::new(), returncode: 0, metadata: HashMap::new() }) }
        fn get_class_attributes(&self, _path: &FilePath) -> ResponseData { ResponseData { value: None, stdout: String::new(), stderr: String::new(), returncode: 0, metadata: HashMap::new() } }
        fn has_all_export(&self, _path: &FilePath) -> SuccessStatus { SuccessStatus::new(false) }
        fn find_primitive_violations(&self, _path: &FilePath, _primitive_types: &PrimitiveTypeList) -> PrimitiveViolationList { PrimitiveViolationList::new() }
        fn find_unused_imports(&self, _path: &FilePath) -> ImportInfoList { ImportInfoList::new() }
        fn get_class_definitions(&self, _path: &FilePath) -> Result<MetadataVO, SourceParserError> { Ok(MetadataVO::new(HashMap::new())) }
        fn get_function_definitions(&self, _path: &FilePath) -> MetadataVO { MetadataVO::new(HashMap::new()) }
        fn is_symbol_exported(&self, _path: &FilePath, _symbol: &SymbolName) -> SuccessStatus { SuccessStatus::new(false) }
        fn get_class_methods(&self, _path: &FilePath) -> MetadataVO { MetadataVO::new(HashMap::new()) }
        fn get_class_bases_map(&self, _path: &FilePath) -> MetadataVO { MetadataVO::new(HashMap::new()) }
        fn get_assignment_targets(&self, _path: &FilePath) -> MetadataVO { MetadataVO::new(HashMap::new()) }
        fn get_control_flow_count(&self, _path: &FilePath) -> Count { Count::new(0) }
        fn is_barrel_file(&self, _path: &FilePath) -> BooleanVO { BooleanVO::new(false) }
        fn get_stem(&self, _path: &FilePath) -> SymbolName { SymbolName::new(String::new()) }
        fn is_entry_point(&self, _path: &FilePath) -> BooleanVO { BooleanVO::new(false) }
        fn get_supported_extensions(&self) -> PatternList { PatternList { values: vec![] } }
    }
    ...
    impl IFileSystemPort for MockFileSystemPort {
        async fn walk(&self, _path: &FilePath, _ignored_patterns: Option<&PatternList>) -> FilePathList { FilePathList { values: vec![] } }
        async fn is_directory(&self, _path: &FilePath) -> SuccessStatus { SuccessStatus::new(false) }
        async fn is_file(&self, _path: &FilePath) -> SuccessStatus { SuccessStatus::new(false) }
        async fn get_relative_path(&self, _path: &FilePath, _start: &FilePath) -> FilePath { FilePath::new(String::new()).unwrap() }
        async fn read_text(&self, _path: &FilePath) -> Result<crate::common::taxonomy_source_vo::ContentString, crate::file_system::taxonomy_filesystem_error::FileSystemError> { Ok(crate::common::taxonomy_source_vo::ContentString::new(String::new())) }
        async fn get_line_count(&self, _path: &FilePath) -> Count { Count::new(0) }
        async fn exists(&self, _path: &FilePath) -> SuccessStatus { SuccessStatus::new(false) }
        async fn get_parent(&self, _path: &FilePath) -> FilePath { FilePath::new(String::new()).unwrap() }
        async fn write_text(&self, _path: &FilePath, _content: &crate::common::taxonomy_source_vo::ContentString, _mode: Option<&crate::common::taxonomy_layer_vo::Identity>) -> Result<SuccessStatus, crate::file_system::taxonomy_filesystem_error::FileSystemError> { Ok(SuccessStatus::new(true)) }
        async fn glob(&self, _pattern: &crate::common::taxonomy_layer_vo::Identity) -> FilePathList { FilePathList { values: vec![] } }
        async fn get_cwd(&self) -> FilePath { FilePath::new(String::new()).unwrap() }
        async fn get_basename(&self, _path: &FilePath) -> crate::common::taxonomy_layer_vo::Identity { crate::common::taxonomy_layer_vo::Identity::new("") }
        async fn path_join(&self, _parts: &[crate::common::taxonomy_layer_vo::Identity]) -> FilePath { FilePath::new(String::new()).unwrap() }
        async fn read_file(&self, _path: &FilePath) -> Result<crate::common::taxonomy_source_vo::ContentString, crate::file_system::taxonomy_filesystem_error::FileSystemError> { Ok(crate::common::taxonomy_source_vo::ContentString::new(String::new())) }
    }
    ...
    impl IAnalyzer for MockAnalyzer {
        fn config(&self) -> &ArchitectureConfig { &self.config }
        fn layer_map(&self) -> &LayerMapVO { &self.layer_map }
        fn fs(&self) -> &dyn IFileSystemPort { &self.fs }
        fn parser(&self) -> &dyn ISourceParserPort { &self.parser }
        fn detect_layer(&self, _f: &FilePath, _root_dir: &FilePath) -> Option<LayerNameVO> { None }
        fn detect_module_layer(&self, _module_path: &FilePath) -> Option<LayerNameVO> { None }
    }
    ...
    async fn test_check_cycles_detection() {
        ...
        let analyzer = MockAnalyzer { parser, config: ArchitectureConfig::default(), layer_map: LayerMapVO::new(HashMap::new()), fs: MockFileSystemPort { _dummy: false } };

        let files = FilePathList { values: vec![FilePath::new("/src/a.rs".to_string()).unwrap(), FilePath::new("/src/b.rs".to_string()).unwrap()] };
        ...
        protocol.check_cycles(&analyzer, &files, &root_dir, &mut results).await;

        assert!(!results.values.is_empty());
        assert_eq!(&*results.values[0].code, "AES015");
    }
    ...
    async fn test_check_cycles_self_cycle() {
        ...
        let files = FilePathList { values: vec![FilePath::new("/src/a.rs".to_string()).unwrap()] };
        let root_dir = FilePath::new("/".to_string()).unwrap();
        let mut results = LintResultList::default();
        ...
        assert!(!results.values.is_empty(), "Should detect self circular dependency");
        let result = &results.values[0];
        assert_eq!(&*result.code, "AES015");
        ...
        fn extract_imports(&self, path: &FilePath) -> Result<ImportInfoList, SourceParserError> {
            ...
            if let Some(modules) = self.imports.get(path.value()) {
                for (i, module) in modules.iter().enumerate() {
                    list.push(ImportInfo::new(LineNumber::new((i + 1) as i64), module.clone()));
                }
            }
            Ok(list)
        }
        fn get_raw_symbols(&self, _path: &FilePath) -> Result<crate::pipeline_jobs::taxonomy_job_vo::ResponseData, SourceParserError> { todo!() }
        fn get_class_attributes(&self, _path: &FilePath) -> crate::pipeline_jobs::taxonomy_job_vo::ResponseData { todo!() }
        fn has_all_export(&self, _path: &FilePath) -> crate::pipeline_jobs::taxonomy_job_vo::SuccessStatus { todo!() }
        fn find_primitive_violations(&self, _path: &FilePath, _primitive_types: &crate::language_adapters::taxonomy_naming_list_vo::PrimitiveTypeList) -> crate::code_analysis::taxonomy_import_source_vo::PrimitiveViolationList { todo!() }
        fn find_unused_imports(&self, _path: &FilePath) -> ImportInfoList { todo!() }
        fn get_class_definitions(&self, _path: &FilePath) -> Result<crate::common::taxonomy_suggestion_vo::MetadataVO, SourceParserError> { todo!() }
        fn get_function_definitions(&self, _path: &FilePath) -> crate::common::taxonomy_suggestion_vo::MetadataVO { todo!() }
        fn is_symbol_exported(&self, _path: &FilePath, _symbol: &crate::common::taxonomy_name_vo::SymbolName) -> crate::pipeline_jobs::taxonomy_job_vo::SuccessStatus { todo!() }
        fn get_class_methods(&self, _path: &FilePath) -> crate::common::taxonomy_suggestion_vo::MetadataVO { todo!() }
        fn get_class_bases_map(&self, _path: &FilePath) -> crate::common::taxonomy_suggestion_vo::MetadataVO { todo!() }
        fn get_assignment_targets(&self, _path: &FilePath) -> crate::common::taxonomy_suggestion_vo::MetadataVO { todo!() }
        fn get_control_flow_count(&self, _path: &FilePath) -> crate::common::taxonomy_common_vo::Count { todo!() }
        fn is_barrel_file(&self, _path: &FilePath) -> crate::common::taxonomy_common_vo::BooleanVO { todo!() }
        fn get_stem(&self, _path: &FilePath) -> crate::common::taxonomy_name_vo::SymbolName { todo!() }
        fn is_entry_point(&self, _path: &FilePath) -> crate::common::taxonomy_common_vo::BooleanVO { todo!() }
        fn get_supported_extensions(&self) -> crate::common::taxonomy_common_vo::PatternList { todo!() }
    }
    ...
    impl IAnalyzer for MockAnalyzerForCycle {
        fn config(&self) -> &ArchitectureConfig { todo!() }
        fn layer_map(&self) -> &LayerMapVO { todo!() }
        fn fs(&self) -> &dyn IFileSystemPort { todo!() }
        fn parser(&self) -> &dyn ISourceParserPort {
            &self.parser
        }
        fn detect_layer(&self, _f: &FilePath, _root_dir: &FilePath) -> Option<LayerNameVO> { todo!() }
        fn detect_module_layer(&self, _module_path: &FilePath) -> Option<LayerNameVO> { todo!() }
    }
    ...
    async fn test_happy_path_no_cycles() {
        ...
        let analyzer = MockAnalyzerForCycle { parser: MockParserForCycle { imports } };
        ...
        checker.check_cycles(&analyzer, &files, &root_dir, &mut results).await;

        assert!(results.values.is_empty(), "Expected no cycles, found: {:?}", results.values);
    }
    ...
    async fn test_self_cycle() {
        ...
        assert!(msg.contains("/src/a.rs -> /src/a.rs"), "Expected A->A cycle, got: {}", msg);
    }
    ...
    async fn test_simple_cycle() {
        ...
        assert_eq!(results.values.len(), 1);
        let msg = &results.values[0].message.value;
        assert!(msg.contains("/src/a.rs -> /src/b.rs -> /src/a.rs") || msg.contains("/src/b.rs -> /src/a.rs -> /src/b.rs"), "Got message: {}", msg);
    }
    ...
    async fn test_js_ts_relative_and_barrel_imports() {
        ...
        assert_eq!(results.values.len(), 1, "Expected exactly 1 cycle, found: {:?}", results.values);
        let msg = &results.values[0].message.value;
        assert!(msg.contains("button.ts") && msg.contains("index.ts") && msg.contains("helper.ts"), "Got message: {}", msg);
    ...
    async fn test_python_relative_imports() {
        ...
        assert_eq!(results.values.len(), 1, "Expected cycle, got: {:?}", results.values);
