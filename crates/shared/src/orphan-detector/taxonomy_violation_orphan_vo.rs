    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            AesOrphanViolation::TaxonomyOrphan { reason } => {
                let default_why =
                    "Taxonomy value objects must be referenced by contract layer modules."
                        .to_string();
                let why = reason
                    .as_ref()
                    .map(|r| r.to_string())
                ...
                write!(
                    f,
                    "AES501 TAXONOMY_ORPHAN: Taxonomy code is unreachable.\n\
                        WHY? {}\n\
                        FIX: Wire the taxonomy file into its contract layer implementation.",
                    why
                )
            }
            AesOrphanViolation::CapabilitiesOrphan { reason } => {
                let default_why =
                    "Capabilities must be registered in the root container to be discoverable."
                        .to_string();
                let why = reason
                    .as_ref()
                    .map(|r| r.to_string())
                ...
                write!(
                    f,
                    "AES503 CAPABILITIES_ORPHAN: Capabilities implementation is not wired.\n\
                        WHY? {}\n\
                        FIX: Register this capability in the root container or orchestrator.",
                    why
                )
            }
            AesOrphanViolation::AgentOrphan { reason } => {
                let default_why =
                    "Agent components must serve as the entry point for capability operations."
                        .to_string();
                let why = reason
                    .as_ref()
                    .map(|r| r.to_string())
