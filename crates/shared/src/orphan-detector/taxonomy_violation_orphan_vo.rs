...
impl fmt::Display for AesOrphanViolation {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            AesOrphanViolation::TaxonomyOrphan { stem, category, reason } => {
                ...
                let why = reason
                    .as_ref()
                    .map(|r| r.to_string())
                    .unwrap_or_else(|| format!("Taxonomy file '{}' is not imported by any file.", stem));
                write!(f, "AES501 TAXONOMY_ORPHAN: '{}' is not imported.\nWHY? {}\nFIX: Import '{}' in {}.", stem, why, stem, target_hint)
            }
            AesOrphanViolation::ContractOrphan { suffix, trait_name, target_layer, reason } => {
                let why = reason
                    .as_ref()
                    .map(|r| r.to_string())
                    .unwrap_or_else(|| format!("Contract {} '{}' is not implemented by any {} file.", suffix, trait_name, target_layer));
                ...
                write!(f, "AES502 CONTRACT_ORPHAN: Contract {} '{}' is orphaned.\nWHY? {}\nFIX: {}", suffix, trait_name, why, fix)
            }
            AesOrphanViolation::CapabilitiesOrphan { stem, reason } => {
                let why = reason
                    .as_ref()
                    .map(|r| r.to_string())
                    .unwrap_or_else(|| format!("Capabilities file '{}' is not wired in any container.", stem));
            ...
            AesOrphanViolation::AgentOrphan { agg_name, reason } => {
                let why = reason
                    .as_ref()
                    .map(|r| r.to_string())
                    .unwrap_or_else(|| format!("Agent aggregate '{}' is not called by any surface or container.", agg_name));
            ...
            AesOrphanViolation::SurfaceOrphan { category, stem, reason } => {
                ...
                let why = reason
                    .as_ref()
                    .map(|r| r.to_string())
                    .unwrap_or_else(|| format!("{} surface '{}' is not imported by any {}.", category, stem, where_hint));
