...
/// Recursively extract identifiers from a token stream (for macro bodies).
pub fn extract_idents_from_stream(stream: proc_macro2::TokenStream, out: &mut Vec<String>) {
    for tt in stream {
        match tt {
