...
use shared::config_system::contract_workspace_detector_port::{IWorkspaceDetectorPort, WorkspaceType};
...
impl ICodeAnalysisAggregate for MockCodeAnalysis {
    fn run_code_analysis(&self, _: &str) -> LintResultList { LintResultList::default() }
    fn run_code_analysis_dir(&self, _: &str) -> LintResultList { LintResultList::default() }
...
fn config_show_with_orchestrator_returns_rules_and_layers() {
    ...
    assert!(result.output.contains("[presentation]"), "missing presentation scope");
