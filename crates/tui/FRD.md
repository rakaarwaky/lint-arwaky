...
├── preview_mode: preview mode (file content | lint results | help overlay | action output)
├── panel_focus: panel focus (tree | file list | preview)
├── terminal_height: u16
├── terminal_width: u16
├── show_path_dialog: boolean
├── show_help: boolean
├── search_mode: boolean
├── search_query: string
├── filtered_indices: list of indices
├── scanning: boolean
├── should_quit: boolean
...
├── action check | action scan | action fix | action ci | action orphan
├── action security | action duplicates | action dependencies
├── action doctor | action init | action install | action mcp config
├── action config show | action install hook | action uninstall hook
├── action adapters | action version | action watch
├── toggle help | toggle search
├── search input | search backspace | search confirm | search cancel
├── path input | path backspace | path confirm | path use current
├── copy to clipboard | copy to file
