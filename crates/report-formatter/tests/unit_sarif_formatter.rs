...
// ─── format: PARSE_WARN diagnostics included as SARIF results ──

#[test]
fn sarif_formatter_includes_diagnostics() {
    let formatter = formatter();
    ...
    let result = formatter.format(&report, Format::Sarif);
    assert!(result.value.contains("PARSE_WARN"));
    assert!(result.value.contains("File skipped: parse error"));
}

// ─── format: Rules array present ──

#[test]
fn sarif_formatter_includes_rules_array() {
    let formatter = formatter();
    let results = vec![LintResult::new_arch_with_column(
        "test.rs",
        1,
    ...
    let result = formatter.format(&report, Format::Sarif);
    assert!(result.value.contains("\"rules\""));
    assert!(result.value.contains("AES201"));
}

// ─── format: Line number 0 clamped to 1 ──

#[test]
fn sarif_formatter_clamps_line_number_zero() {
    let formatter = formatter();
    ...
    assert!(result.value.contains("\"start_line\": 1"));
}
