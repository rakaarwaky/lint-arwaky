...
fn make_result_produces_lint_result() {
    let result = NamingConventionChecker::make_result("test.rs", "AES101", "naming violation", Severity::HIGH);
...
fn check_file_naming_skips_barrel_files() {
    ...
    checker.check_file_naming("mod.rs", "mod.rs", &None, None, &Default::default(), &mut violations);
...
fn check_file_naming_skips_entry_points() {
    let checker = NamingConventionChecker::new();
    let mut violations = vec![];
    checker.check_file_naming("main.rs", "main.rs", &None, None, &Default::default(), &mut violations);
...
fn check_file_naming_reports_unknown_prefix() {
    let checker = NamingConventionChecker::new();
    let mut violations = vec![];
    checker.check_file_naming("bad_prefix_checker.rs", "bad_prefix_checker.rs", &None, None, &Default::default(), &mut violations);
...
fn check_file_naming_reports_no_layer() {
    let checker = NamingConventionChecker::new();
    let mut violations = vec![];
    checker.check_file_naming("file.rs", "file.rs", &None, None, &Default::default(), &mut violations);
    assert!(!violations.is_empty(), "file without layer prefix should be flagged");
}
...
fn check_file_naming_valid_layer_file() {
    ...
    assert!(violations.is_empty(), "valid layer file should not be flagged");
}
