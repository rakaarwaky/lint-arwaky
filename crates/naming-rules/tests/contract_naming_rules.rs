...
fn naming_orchestrator_name_returns_expected() {
    let config = std::sync::Arc::new(ArchitectureConfig::default());
    let layer_map = std::sync::Arc::new(LayerMapVO::new(std::collections::HashMap::new()));
...
use naming_rules_lint_arwaky::agent_naming_orchestrator::NamingOrchestratorDeps;
