...
impl DummyImportChecker {
    ...
    fn _detect_layer(&self, file: &str, layer_map: &shared::taxonomy_definition_vo::LayerMapVO) -> String {
    ...
    fn _check_dummy_imports(&self, file: &str, content: &str, violations: &mut Vec<LintResult>, layer_map: &shared::taxonomy_definition_vo::LayerMapVO) {
        ...
        for (symbol, line_no) in utility_dummy_detector::imported_symbols(&lines, lang) {
            ...
            if utility_dummy_detector::symbol_used_real(&lines, &symbol_str, &dummy_ranges, &dummy_impl_traits) {
                continue;
            }
    ...
    fn _check_dummy_functions(&self, file: &str, content: &str, violations: &mut Vec<LintResult>, layer_map: &shared::taxonomy_definition_vo::LayerMapVO) {
        let lines: Vec<&str> = content.lines().collect();
    ...
    fn _check_dummy_impls(&self, file: &str, content: &str, violations: &mut Vec<LintResult>, layer_map: &shared::taxonomy_definition_vo::LayerMapVO) {
        let lines: Vec<&str> = content.lines().collect();
        let layer_name = self._detect_layer(file, layer_map);
    ...
    fn _check_taxonomy_intent(&self, file: &str, content: &str, violations: &mut Vec<LintResult>, layer_map: &shared::taxonomy_definition_vo::LayerMapVO) {
        ...
        let has_real_usage = imported.iter().any(|(symbol, line_no)| {
            ...
            utility_dummy_detector::symbol_used_real(&lines, symbol.value(), &dummy_ranges, &dummy_impl_traits)
        });
