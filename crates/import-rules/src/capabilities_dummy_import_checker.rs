...
impl DummyImportChecker {
    ...
    fn check_dummy_imports(&self, file: &str, content: &str, violations: &mut Vec<LintResult>, analyzer: &dyn IAnalyzer, root_dir: &FilePath) {
    ...
    fn check_dummy_functions(&self, file: &str, content: &str, violations: &mut Vec<LintResult>, analyzer: &dyn IAnalyzer, root_dir: &FilePath) {
        let lines: Vec<&str> = content.lines().collect();
        let lang = Language::from_path(file);
    ...
    fn check_dummy_impls(&self, file: &str, content: &str, violations: &mut Vec<LintResult>, analyzer: &dyn IAnalyzer, root_dir: &FilePath) {
        let lines: Vec<&str> = content.lines().collect();
    ...
    fn check_taxonomy_intent(&self, file: &str, content: &str, violations: &mut Vec<LintResult>, analyzer: &dyn IAnalyzer, root_dir: &FilePath) {
        let lines: Vec<&str> = content.lines().collect();
        let lang = Language::from_path(file);
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            let is_dummy = match lang {
                ...
                Language::Python => {
                    trimmed.starts_with("def _use_") && trimmed.contains("()")
                }
                Language::JavaScript => {
        ...
        let taxonomy_primitives = [
            "LineNumber", "ColumnNumber", "Count", "Score", "String", "bool", "i32", "u32",
            "f64", "usize",
        ];
    ...
    fn check_aggregate_intent(&self, file: &str, content: &str, violations: &mut Vec<LintResult>) {
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            let is_phantom = match lang {
                ...
                Language::JavaScript => trimmed.contains("@ts-ignore") || trimmed.contains("@ts-expect"),
                Language::Unknown => false,
            };
