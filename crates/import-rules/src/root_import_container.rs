...
impl ImportContainer {
    pub fn new() -> Self {
        ...
        let fs = Arc::new(file_system::infrastructure_filesystem_adapter::OSFileSystemAdapter::new());
        let source_parser = Arc::new(source_parsing::infrastructure_parser_adapter::SourceParserOrchestrator::new(
        ...
        let analyzer = Arc::new(crate::capabilities_layer_detection_analyzer::LayerDetectionAnalyzer::new(
            config,
            fs,
            source_parser,
        ));
        ...
        let mandatory = Arc::new(
            crate::capabilities_import_mandatory_checker::ArchImportMandatoryChecker::new(parser.clone()),
        );
        let forbidden = Arc::new(
            crate::capabilities_import_forbidden_checker::ArchImportForbiddenChecker::new(parser.clone()),
        );
        let intent = Arc::new(
            crate::capabilities_import_intent_checker::ImportIntentChecker::new(parser),
        );
    ...
    pub fn orchestrator(&self) -> Arc<dyn IImportRunnerAggregate> {
        Arc::new(
            crate::agent_import_orchestrator::ImportOrchestrator::new(
