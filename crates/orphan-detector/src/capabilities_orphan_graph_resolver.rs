...
mod engine_fix_tests {
    ...
    fn make_resolver() -> OrphanGraphResolver {
        let extractor: Arc<dyn IOrphanFilenameExtractorProtocol> =
            Arc::new(crate::capabilities_orphan_filename_extractor::OrphanFilenameExtractor::new());
        let cache: Arc<dyn IOrphanFileCachePort> =
            Arc::new(crate::infrastructure_file_cache::OrphanFileCache::new());
        OrphanGraphResolver::new(extractor, cache)
    }
    ...
    #[test]
    fn resolve_module_file_links_leaf_module() {
        // Simulates `shared::orphan_detector::taxonomy_orphan_result_utility`
        let base = std::env::temp_dir().join("arwaky_resolve_test");
        let _ = std::fs::remove_dir_all(&base);
        write_file(
            base.join("shared/src/orphan-detector/taxonomy_orphan_result_utility.rs")
                .to_str()
                .unwrap(),
        );
        let src = base.join("shared/src");
        let r = make_resolver();
        let got = r.resolve_module_file(
            "shared",
            &["orphan_detector", "taxonomy_orphan_result_utility"],
            &src,
            "importer.rs",
        );
        let expected = src
    ...
    #[test]
    fn resolve_module_file_tolerates_trailing_function_segment() {
        // Simulates `shared::orphan_detector::taxonomy_orphan_result_utility::mk_orphan_result`
        let base = std::env::temp_dir().join("arwaky_resolve_test2");
        let _ = std::fs::remove_dir_all(&base);
        write_file(
            base.join("shared/src/orphan-detector/taxonomy_orphan_result_utility.rs")
                .to_str()
        ...
        let got = r.resolve_module_file(
            ...
            &[
                "orphan_detector",
                "taxonomy_orphan_result_utility",
                "mk_orphan_result",
            ],
            &src,
            "importer.rs",
