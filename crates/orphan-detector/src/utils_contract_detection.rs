...
pub fn has_ts_impl(content: &str, trait_name: &str) -> bool {
    let re = super::utils_contract_regex::word_boundary_re(trait_name);
    for line in content.lines() {
        let t = line.trim();
...
pub fn extract_contract_trait_name(content: &str, file_path: &str) -> Option<String> {
    ...
    match ext {
        "rs" => super::utils_contract_regex::re_contract_rust()
            .and_then(|re| re.captures(&code))
            .map(|caps| caps[1].to_string()),
        "py" => {
            if let Some(caps) = super::utils_contract_regex::re_contract_py()
                .and_then(|re| re.captures(&code))
            {
                caps.get(1)
                    .or_else(|| caps.get(2))
                ...
                super::utils_contract_regex::re_contract_py_fallback()
                    .and_then(|re| re.captures(&code))
                    .map(|caps| caps[1].to_string())
            }
        }
        "ts" | "tsx" | "js" | "jsx" => {
            super::utils_contract_regex::re_ts_interface_export()
                .and_then(|re| re.captures(&code))
                .or_else(|| {
                    super::utils_contract_regex::re_interface()
                        .and_then(|re| re.captures(&code))
                })
                .map(|caps| caps[1].to_string())
        }
