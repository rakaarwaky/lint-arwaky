...
fn suffix_from_command_file() {
    ...
    let suffix = SurfacesOrphanAnalyzer::get_surface_suffix("surface_check_command.rs", &extractor);
...
fn suffix_from_hook_file() {
    let extractor = get_extractor();
    let suffix = SurfacesOrphanAnalyzer::get_surface_suffix("surface_use_auth_hook.ts", &extractor);
...
fn suffix_from_entry_file() {
    let extractor = get_extractor();
    let suffix = SurfacesOrphanAnalyzer::get_surface_suffix("root_cli_entry.rs", &extractor);
...
fn category_unknown_for_unrecognized() {
    assert_eq!(SurfacesOrphanAnalyzer::surface_category("xyz"), "unknown");
    assert_eq!(SurfacesOrphanAnalyzer::surface_category(""), "unknown");
}
