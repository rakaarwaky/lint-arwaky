...
pub fn handle_scan_quality(
    ...
    let is_specific_member = std::path::Path::new(&root).parent()
        .map(|p| {
            let parent_name = p.file_name().map(|n| n.to_string_lossy()).unwrap_or_default();
            parent_name == "crates" || parent_name == "modules" || parent_name == "packages"
        })
        .unwrap_or(false);
    output_violations(&violations, &root, format, is_specific_member);
