...
impl CheckCommandsSurface {
    ...
    pub fn scan(&self, path: &str, filter: Option<&str>) {
        ...
        let naming_results = rt.block_on(self.naming_orchestrator.run_audit(&path_obj));
        ...
        let role_results = rt.block_on(self.role_orchestrator.run_audit(&path_obj));
    ...
    pub fn scan_with_discovery(&self, path: &str, filter: Option<&str>) {
        ...
        for ws in &workspaces {
            ...
            let aes_results = self.arch_linter.run_self_lint(&ws.path.value);
            all_results.extend(aes_results.values);

            let naming_results = rt.block_on(self.naming_orchestrator.run_audit(&ws.path));
            all_results.extend(naming_results);

            let import_results = rt.block_on(self.import_orchestrator.run_audit(&ws.path));
            all_results.extend(import_results);
            ...
            let role_results = rt.block_on(self.role_orchestrator.run_audit(&ws.path));
            all_results.extend(role_results);
