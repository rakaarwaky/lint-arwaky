...
pub async fn handle_config_show(orchestrator: Arc<dyn IConfigOrchestratorAggregate>) -> ExitCode {
    let project_root = FilePath::new(".".to_string()).unwrap_or_default();

    match orchestrator.list_config_files(&project_root).await {
        Ok(config_files) if !config_files.is_empty() => {
            for (lang, path) in &config_files {
                match orchestrator.read_config(&project_root, *lang).await {
                    Ok(Some(source)) => {
