// PURPOSE: AgentRoleChecker — IAgentRoleChecker for AES405: agent container/orchestrator/lifecycle role audits
...
pub fn aes405_must_implement_contract(contract_name: &str) -> String {
    format!(
        "AES405 AGENT_ROLE: Class must implement '{}'.",
        contract_name
    )
}

pub fn aes405_any_type(line: &str) -> String {
    format!(
        "AES405 AGENT_ROLE: Any type annotation found in agent orchestrator layer: '{}'.",
        line.trim()
    )
}
...
impl AgentRoleChecker {
    ...
    fn _check_must_implement_contract_lazy(
        ...
        results: &mut shared::cli_commands::taxonomy_result_vo::LintResultList,
    ) {
        let contract_name = SymbolName::new("ServiceContainerAggregate");
        self._check_must_implement_contract(
            f,
            &contract_name,
            AesRoleViolation::MustImplementContract { reason: None }.to_string(),
            analyzer,
            results,
            "AES405",
        );
            ...
            if let Some(ref name) = method_name {
                if name.value != "__init__" {
                    results.push(LintResult {
                        ...
                        code: ErrorCode::raw("AES405"),
                        message: LintMessage::new(AesRoleViolation::StatelessExecution {
                            reason: None,
                        }),
        ...
        for imp in imports.values {
            if imp.module.contains(&layer_infrastructure().value) {
                results.push(LintResult {
                    ...
                    column: ColumnNumber::new(0),
                    code: ErrorCode::raw("AES405"),
                    message: LintMessage::new(AesRoleViolation::HighLevelPolicy { reason: None }),
                    source: Some(AdapterName::raw("architecture")),
                    severity: Severity::HIGH,
                    enclosing_scope: None,
            ...
            if let Some(ref init_m) = init_method {
                if self._count_orchestrator_args(init_m) < 2 {
                    ...
                    results.push(LintResult {
                        ...
                        column: ColumnNumber::new(0),
                        code: ErrorCode::raw("AES405"),
                        message: LintMessage::new(AesRoleViolation::CoordinatesMultiple {
                            reason: None,
                        }),
                        source: Some(AdapterName::raw("architecture")),
            ...
            if init_method.is_some() {
                ...
                if control_flow_count.value > 2 {
                    ...
                    results.push(LintResult {
                        ...
                        column: ColumnNumber::new(0),
                        code: ErrorCode::raw("AES405"),
                        message: LintMessage::new(AesRoleViolation::LazyEagerInit { reason: None }),
                        source: Some(AdapterName::raw("architecture")),
                        severity: Severity::HIGH,
                        enclosing_scope: None,
                        related_locations: shared::taxonomy_lint_vo::LocationList::new(),
    ...
    fn _check_must_implement_contract(
        ...
        for bases in bases_map.value.values() {
            ...
            if !has_contract {
                let message = aes405_must_implement_contract(&contract_name.value);
                results.push(LintResult {
                    file: f.clone(),
                    line: LineNumber::new(0),
                    column: ColumnNumber::new(0),
            ...
            if let Some(ref re) = *ANY_TYPE_RE {
                for mat in re.find_iter(line) {
                    ...
                    results.push(LintResult {
                        ...
                        code: ErrorCode::raw("AES405"),
                        message: LintMessage::new(aes405_any_type(line)),
                        source: Some(AdapterName::raw("architecture")),
                        severity: Severity::HIGH,
                        enclosing_scope: None,
                        related_locations: shared::taxonomy_lint_vo::LocationList::new(),
