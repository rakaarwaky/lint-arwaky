...
impl IHookProtocol for HookManager {
    async fn install_pre_commit(&self, executable_path: &FilePath) -> Result<SuccessStatus, GitHookError> {
    ...
    async fn get_diff_data(&self, path1: &str, path2: &str) -> std::collections::HashMap<String, serde_json::Value> {
    ...
    async fn get_suggestions(&self, path: &str) -> std::collections::HashMap<String, serde_json::Value> {
        let mut result = std::collections::HashMap::new();
