// Utility layer — stateless functions for default configuration values
...
pub fn default_aes_config() -> ArchitectureConfig {
    DEFAULT_CONFIG
        .get_or_init(|| {
            parse_config_yaml(include_str!("../../shared/config/lint_arwaky.config.yaml"))
        })
        .clone()
}
...
fn parse_config_yaml_with_warnings(yaml_str: &str) -> (ArchitectureConfig, Vec<String>) {
