...
fn acceptance_check_action_on_current_project() {
    ...
    for violation in &result.values {
        assert!(!violation.aes_code.is_empty(), "Each violation should have an AES code");
    }
}
...
fn acceptance_output_component_formats_results() {
    ...
    let output = dispatcher_lint_arwaky::surface_output_component::OutputComponent::format(&results);
