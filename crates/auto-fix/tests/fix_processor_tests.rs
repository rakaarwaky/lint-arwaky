...
impl ICodeAnalysisAggregate for MockLinter {
    fn run_code_analysis(&self, _path: &str) -> LintResultList { LintResultList::default() }
    fn run_code_analysis_dir(&self, _src_dir: &str) -> LintResultList { LintResultList::default() }
    fn run_code_analysis_path(&self, _path: &str) -> Vec<LintResult> { vec![] }
    fn calc_score(&self, _results: &[LintResult]) -> f64 { 0.0 }
    fn check_critical(&self, _results: &[LintResult]) -> bool { false }
    fn format_report(&self, _results: &LintResultList, _project_root: &str) -> String { String::new() }
    fn active_rules(&self) -> Vec<CodeAnalysisRuleVO> { vec![] }
}

fn make_result(code: &str) -> LintResult {
    LintResult {
        file: shared::common::taxonomy_path_vo::FilePath::new("test.rs".to_string()).unwrap_or_default(),
...
fn fixable_codes_includes_expected() {
    let processor = LintFixProcessor::with_dry_run(true, Arc::new(MockLinter) as Arc<dyn ICodeAnalysisAggregate>);
...
fn report_non_fixable_filters_correctly() {
    let processor = LintFixProcessor::with_dry_run(true, Arc::new(MockLinter) as Arc<dyn ICodeAnalysisAggregate>);
    let violations = vec![make_result("AES101"), make_result("AES500"), make_result("AES203")];
