...
fn build_surface() -> LintArwakyMcpServer {
    ...
    LintArwakyMcpServer::new(std::sync::Arc::new(orchestrator))
}
...
fn bench_version_command(c: &mut Criterion) {
    ...
    group.bench_function("version", |b| {
        b.iter(|| {
            ...
            std::hint::black_box(rt.block_on(async { surface.execute_command(args).await }))
        })
    });
}
    ...
    for domain in [None, Some("check"), Some("hook")] {
        ...
        group.bench_with_input(BenchmarkId::new("filter", label), &domain, |b, d| {
            b.iter(|| {
                ...
                std::hint::black_box(rt.block_on(async { surface.list_commands(args).await }))
            })
        });
    }
    group.finish();
...
fn bench_health_check(c: &mut Criterion) {
    ...
    group.bench_function("check", |b| {
        b.iter(|| std::hint::black_box(rt.block_on(async { surface.health_check().await })))
    });
}
