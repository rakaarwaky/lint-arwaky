...
pub struct McpServerDependencies {
    pub code_analysis_linter:
        Arc<dyn shared::quality_rules::contract_code_analysis_aggregate::ICodeAnalysisAggregate>,
    pub fix_orchestrator: Arc<dyn LintFixOrchestratorAggregate>,
    pub orphan_orchestrator:
        Arc<dyn shared::orphan_rules::contract_orphan_aggregate::IOrphanAggregate>,
    ...
    fn execute_command(&self, Parameters(args): Parameters<ExecuteCommandArgs>) -> String {
        ...
        let result = match action.as_str() {
            "check" | "scan" => {
                ...
                let orphan_analyzer = orphan_rules::root_orphan_detector_container::OrphanContainer::from_orchestrator(
            ...
            "doctor" => {
                ...
                let diag = self.deps.maintenance_orchestrator.diagnose_toolchain();
            ...
            "security" => {
                ...
                let report = self.deps.maintenance_orchestrator.run_security_scan(&fp);
            ...
            "duplicates" => {
                // Run code duplication analysis via quality-rules aggregate
                let path = match arg_path {
                    Some(p) => p,
                    None => ".".to_string(),
                };
            ...
            "uninstall-hook" => {
                ...
                match self.deps.git_hooks_aggregate.uninstall_hook() {
                    Ok(_) => {
            ...
            "install" => {
                ...
                let py_status = self.deps.setup_orchestrator.install_python_adapters();
                let js_status = self
                    .deps
                    .setup_orchestrator
            ...
            "config-show" => {
                ...
                match self.deps.config_orchestrator.list_config_files(&fp) {
                    Ok(config_files) if !config_files.is_empty() => {
                        ...
                        for (lang, path) in &config_files {
                            ...
                            let source = match self.deps.config_orchestrator.read_config(&fp, *lang)
                            {
            ...
            "quality" => {
                // Run code quality analysis directly via quality-rules aggregate
                let path = match arg_path {
                    Some(p) => p,
                    None => ".".to_string(),
                };
            ...
            "import" => {
                ...
                let results = match self.deps.import_orchestrator.run_audit(&fp) {
                    Ok(r) => r,
            ...
            "role" => {
                ...
                // Validate path
                if shared::common::taxonomy_path_vo::FilePath::new(path.clone()).is_err() {
                    return serde_json::json!({"error": "Invalid path", "action": "role", "exit_code": 2}).to_string();
                }
                // IRoleRunnerAggregate only has run_audit_with_entries — use cached file list
                let results = self
                    .deps
                    .role_orchestrator
                    .run_audit_with_entries(self.deps.filesystem.file_list());
    ...
    fn get_config(&self, Parameters(args): Parameters<GetConfigArgs>) -> String {
        ...
        for (lang, _config_path) in &config_files {
            ...
            if let Ok(Some(source)) = self.deps.config_orchestrator.read_config(&fp, *lang) {
                ...
                let arch_config =
                    config_system::utility_config_parser::parse_config_yaml(&source.raw_content);
                ...
                if score_threshold.is_none()
                    && let Some(t) = config_system::utility_config_parser::parse_score_threshold(
                        &source.raw_content,
                    )
                {
