# Cleanup & Consolidate — Rust (full procedure)

> Per-language half of the [`cleanup-consolidate`](../SKILL.md) skill. `../SKILL.md` holds the shared methodology (two-phase rule, workflow skeleton, report format, dry-run); this file is the complete, self-contained Rust procedure — rules, detection patterns, tool commands, checklists.
