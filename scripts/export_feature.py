...
Usage:
    ...
    python3 scripts/export_feature.py --crate code-analysis
    python3 scripts/export_feature.py --crate import-rules --output /tmp/out.md
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export a feature crate into a single consolidated Markdown file."
    )
    parser.add_argument(
        "--crate", "-c",
        help="Crate name to export (non-interactive mode). Omit for interactive selection.",
    )
    parser.add_argument(
        "--output", "-o",
        help="Output file path (default: .agents/finding/<crate>_v<ver>.md).",
    )
...
def main() -> None:
    ...
    if args.crate:
        ...
        if args.crate not in feature_crates:
            print(f"Error: Crate '{args.crate}' not found. Available: {', '.join(feature_crates)}", file=sys.stderr)
            sys.exit(1)

        selected_crate = args.crate
        print(f"Processing crate: {selected_crate}...")
