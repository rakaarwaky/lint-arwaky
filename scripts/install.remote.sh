...
# 3. Install binaries — try GitHub Release first (no cargo/build required)
...
if curl -fsSL "$DOWNLOAD_URL" -o /tmp/lint-arwaky.tar.gz 2>/dev/null; then
    tar xzf /tmp/lint-arwaky.tar.gz -C "$INSTALL_BIN" lint-arwaky-cli lint-arwaky-mcp lint-arwaky-tui 2>/dev/null && {
        ...
        echo -e "  ${GREEN}✓ No cargo or build required!${NC}"
        rm -f /tmp/lint-arwaky.tar.gz
    }
else
    # Fallback: if download fails, try cargo (requires Rust installed)
    echo "  [warn] Pre-built binary unavailable for $ARCH"
    if ! command -v cargo &>/dev/null; then
        echo "  [info] cargo not found. Installing Rust to build from source..."
        install_if_missing cargo "Rust/Cargo" "curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y && . \"\$HOME/.cargo/env\""
    fi

    if command -v cargo &>/dev/null; then
        echo "  Building from crates.io/git..."
        ...
        echo "  [error] cargo not found and pre-built binary unavailable."
        echo "  Please install Rust manually or download binaries from:"
        echo "  https://github.com/rakaarwaky/lint-arwaky/releases"
        exit 1
    fi
fi
